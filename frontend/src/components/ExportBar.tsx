...
import { downloadCSV, downloadJSON, toMarkdownTable, toBestListText, copyToClipboard } from '../utils/export';
...
const ExportBar: React.FC<Props> = ({ listA, listB, listC, date }) => {
  ...
  return (
    <div className="glass-card rounded-xl px-4 py-3 flex flex-wrap gap-2 items-center">
      ...
      <button onClick={() => downloadJSON(all, `forebet-${date}-all.json`)} className={BtnGhost}>
        <DownloadIcon /> JSON
      </button>

      <button onClick={() => copy(toMarkdownTable(listA), 'md-A')} className={BtnGhost}>
        {copied === 'md-A' ? '✓ Copied!' : 'Copy List A (MD)'}
      </button>

      <button onClick={() => copy(toMarkdownTable(listB), 'md-B')} className={BtnGhost}>
        {copied === 'md-B' ? '✓ Copied!' : 'Copy List B (MD)'}
      </button>

      <button
        onClick={() => copy(toBestListText(listC), 'best')}
